...
class KinematicObservation(ObservationType):
    ...
    def observe(self) -> np.ndarray:
        ...
        # Reorder
        df = df[self.features]
        obs = df.values.copy().reshape(1, -1)
...
class TrafficFlowObservationV1(ObservationType):
    ...
    def space(self) -> spaces.Space:
        return spaces.Box(shape=(1, (self.vehicles_count - 1) * 4 + 2), low=-np.inf, high=np.inf, dtype=np.float32)
    ...
    def observe(self) -> Any:
        if not self.env.road:
            return np.zeros(self.space().shape)
        feature = np.zeros((1, (self.vehicles_count - 1) * 4 + 2))
        # Add ego-vehicle
        df = pd.DataFrame.from_records([self.observer_vehicle.to_dict()])
        ego_x, ego_y, ego_vx, ego_vy, ego_heading = float(df['x']), float(df['y']), float(df['vx']), float(df['vy']), float(df['heading']),
        ego_lane = self.env.road.network.get_lane(self.env.vehicle.lane_index)
        ego_longitudinal, ego_lateral = ego_lane.local_coordinates(self.env.vehicle.position)
        ego_lane_heading = ego_lane.heading_at(ego_longitudinal)
        feature[0, 0], feature[0, 1] = ego_vx, ego_vy
        # Add nearby traffic
        close_vehicles = self.close_objects_to(self.observer_vehicle,
                                                        60,
                                                        count=self.vehicles_count - 1,
                                                        see_behind=self.see_behind,
                                                        sort=self.order == "sorted",
        ...
        if close_vehicles:
            for i, v in enumerate(close_vehicles[-self.vehicles_count + 1:]):
                vehicles_df = pd.DataFrame.from_records([v.to_dict()])
                bv_x, bv_y, bv_vx, bv_vy = float(vehicles_df['x']), float(vehicles_df['y']), float(vehicles_df['vx']), float(vehicles_df['vy']),
                feature[0, 2 + i * 4], feature[0, 3 + i * 4], feature[0, 4 + i * 4], feature[0, 5 + i * 4] = (
                    float(ego_x - bv_x), float(ego_y - bv_y), float(ego_vx - bv_vx), float(ego_vy - bv_vy))

        return feature.astype(self.space().dtype)
    ...
    def close_objects_to(self, vehicle: 'kinematics.Vehicle', distance: float, count: Optional[int] = None,
        ...
        vehicles = [v for v in self.env.road.vehicles
                    ...
                    and (see_behind or -2 * vehicle.LENGTH < vehicle.lane_distance_to(v)) and (vehicle.lane_index[1] == v.lane_index[1])]
...
class TrafficFlowObservationV2(ObservationType):
    """Observe the kinematics of nearby vehicles."""
    ...
    def observe(self) -> Any:
        ...
        # Add ego-vehicle
        df = pd.DataFrame.from_records([self.observer_vehicle.to_dict()])
        ego_x, ego_y, ego_vx, ego_vy, ego_heading = float(df['x']), float(df['y']), float(df['vx']), float(
            df['vy']), float(df['heading']),
        ego_lane = self.env.road.network.get_lane(self.env.vehicle.lane_index)
        ego_longitudinal, ego_lateral = ego_lane.local_coordinates(self.env.vehicle.position)
        ego_lane_heading = ego_lane.heading_at(ego_longitudinal)
        feature[0, 0], feature[0, 1] = ego_vx, ego_vy
        ...
        if close_vehicles:
            for i, v in enumerate(close_vehicles[-self.vehicles_count + 1:]):
                vehicles_df = pd.DataFrame.from_records([v.to_dict()])
                bv_x, bv_y, bv_vx, bv_vy = float(vehicles_df['x']), float(vehicles_df['y']), float(
                    vehicles_df['vx']), float(vehicles_df['vy']),
                feature[0, 2 + i * 4], feature[0, 3 + i * 4], feature[0, 4 + i * 4], feature[0, 5 + i * 4] = (
                    float(ego_x - bv_x), float(ego_y - bv_y), float(ego_vx - bv_vx), float(ego_vy - bv_vy))
...
class TrafficFlowObservation(ObservationType):
    ...
    def observe(self) -> Any:
        ...
        return df.to_numpy().reshape(1, self.vehicles_count * len(self.features)).astype(self.space().dtype)
...
def observation_factory(env: 'AbstractEnv', config: dict) -> ObservationType:
    ...
    elif config['type'] == "TrafficFlow":
        return TrafficFlowObservation(env, **config)
    elif config['type'] == "TrafficFlowV1":
        return TrafficFlowObservationV1(env, **config)
    elif config['type'] == "TrafficFlowV2":
        return TrafficFlowObservationV2(env, **config)
