...
class AbstractEnv(gym.Env):
    ...
    def calculate_ttc(self, vehicle):

        if vehicle is None:
            return []

        TTCs = [100]
        agent_dict = vehicle.to_dict()
        p_0 = (agent_dict['x'], agent_dict['y'])
        v_0 = (agent_dict['vx'], agent_dict['vy'])
        ...
        for v in self.road.vehicles:
            ...
            if det == 0:
                ...
                # 采用平行情况的算法
                vector_p = np.array(
                    [p_0[0] - p_i[0], p_0[1] - p_i[1]])
                vector_v = np.array([v_0[0], v_0[1]])
                angle = self.cal_angle(vector_p, vector_v)
                v = np.sqrt(vector_v[0] ** 2 + vector_v[1] ** 2)
                v_projection = v * np.cos(angle / 180 * np.pi)
                if v_projection < 0:
                    thw = np.sqrt(
                        vector_p[0] ** 2 + vector_p[1] ** 2) / v_projection
                    TTCs.append(abs(thw))
            else:
